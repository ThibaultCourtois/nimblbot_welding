...
class WeldingModularControlNode(Node):
    ...
    def _setup_robot_configuration(self) -> None:
        ...
        try:
            ...
            self.__joint_names_alias = [
                alias + str(i) if i != 0 else alias for i in range(self.__num_joints)
            ]

        except (KeyError, TypeError, FileNotFoundError) as e:
            self.get_logger().fatal(
                f'Failed to load robot configuration "{robot_type}" : {e}'
            )
            rclpy.shutdown()
    ...
    def _initialize_basic_parameters(self) -> None:
        ...
        self.current_modular_states = {"turn_module": 0, "wrist_command": 0}
        self.declare_parameter("modular_gain", 1.0)
        self.__q_desired = None
        self.current_speed_vector = None
        self.modular_timer = self.create_timer(
            1 / TIMER_RATE, self._process_modular_control
        )
    ...
    def _create_publishers(self) -> None:
        ...
        self.joints_trajectory_pub = self.create_publisher(
            JointTrajectory, "/nb/desired_trajectory_modular", 10
        )

        self.modular_mode_pub = self.create_publisher(
            String, "/welding_modular_control/modular_mode", 10
        )

        self.modular_velocity_indicator_pub = self.create_publisher(
            Float64, "/welding_modular_control/modular_velocity_indicator", 10
        )

        self.modular_status_pub = self.create_publisher(
            Bool, "/welding_modular_control/status", 10
        )

        self.modular_command_request_pub = self.create_publisher(
            Int8, "/welding_modular_control/control_mode_request", 10
        )
    ...
    def _create_subscriptions(self) -> None:
        ...
        _ = self.create_subscription(
            Float64, "rviz_plugin/modular_gain", self.on_modular_gain_changed, 10
        )
        ...
        _ = self.speed_vector_sub = self.create_subscription(
            Twist, "/welding_command_handler/speed_vector", self.on_speed_vector, 10
        )

        _ = self.joint_trajectory_sub = self.create_subscription(
            JointTrajectory, "/nb/desired_trajectory", self.on_desired_trajectory, 10
        )
    ...
    def _create_clients(self) -> None:
        ...
        """
        # Luos wrapper node
        self.set_zeros_client = self.create_client(Empty_srv, "/nb/set_zeros")
    ...
    def _create_services(self) -> None:
        ...
        self.set_zeros_srv = self.create_service(
            Empty_srv, "welding_cartesian_command/set_zeros_request", self.on_set_zeros
        )
    ...
    def _handle_modular_commands(self) -> None:
        ...
        if toggles["select_module"] == "rise":
            ...
            if self.__modular_selector_mode[0] == MODULE_SELECT_COMMAND:
                self.__modular_selection_module = max(
                    0,
                    min(
                        self.__modular_selection_module + select_module,
                        self.__num_modules - 1,
                    ),
                )
                self.__modular_selection_section = max(
                    0,
                    min(self.__modular_selection_module // 3, self.__num_sections - 1),
                )
            elif self.__modular_selector_mode[0] == SECTION_SELECT_COMMAND:
                self.__modular_selection_section = max(
                    0,
                    min(
                        self.__modular_selection_section + select_module,
        ...
        if toggles["set_speed_module"] == "rise":
            ...
            self.modular_velocity = max(
                MIN_MODULAR_VELOCITY,
                min(
                    self.modular_velocity + set_speed_module * MODULAR_VELOCITY_STEP,
                    MAX_MODULAR_VELOCITY,
                ),
            )
            self.get_logger().info(
                f"Modular velocity adjusted to: {self.modular_velocity:.2f}"
            )

        if toggles["select_command_type"] == "rise":
            self.__modular_command_mode = self.__modular_command_mode[1:] + [
                self.__modular_command_mode[0]
            ]
            self._modular_mode_msg.data = (
                "TILT" if self.__modular_command_mode[0] == TILT else "AZIMUTH"
            )
            self.modular_mode_pub.publish(self._modular_mode_msg)
            self.get_logger().info(
                f"Switched to {'TILT' if self.__modular_command_mode[0] == TILT else 'AZIMUTH'} mode"
            )

        if toggles["selection_mode"] == "rise":
            self.__modular_selector_mode = self.__modular_selector_mode[1:] + [
                self.__modular_selector_mode[0]
            ]
            self.get_logger().info(
                f"Selection mode changed to {'MODULE' if self.__modular_selector_mode[0] == MODULE_SELECT_COMMAND else 'SECTION'}"
            )
    ...
    def _execute_modular_movement(self) -> None:
        ...
        try:
            ...
            if joint_sup_idx > len(self.__q_desired) or joint_inf_idx < 0:
                self.get_logger().error(
                    f"Invalid joint indices: [{joint_inf_idx}:{joint_sup_idx}] for {len(self.__q_desired)} joints"
                )
                return
        ...
        if self.__modular_command_mode[0] == TILT:
            ...
            module_velocities = [
                v if i % 2 == 0 else -v for i, v in enumerate(module_velocities)
            ]

        try:
            if turn_module != 0:
                module_positions = [
                    self.__q_desired[joint_inf_idx + i] + v / ROBOT_RATE
                    for i, v in enumerate(module_velocities)
                ]
            else:
                module_positions = [
                    self.__q_desired[joint_inf_idx + i]
                    for i in range(len(module_velocities))
                ]
        ...
        if self.__terminal_wrist:
            try:
                ...
                if wrist_command != 0:
                    ...
                    wrist_position = (
                        self.__q_desired[wrist_joint_idx] + wrist_velocity / ROBOT_RATE
                    )
                else:
                    wrist_velocity = 0.0
        ...
        self.modular_velocity_indicator_pub.publish(
            self._modular_velocity_indicator_msg
        )
    ...
    def on_modular_gain_changed(self, msg: Float64) -> None:
        ...
        new_gain = msg.data
        self.set_parameters(
            [RclpyParameter("modular_gain", RclpyParameter.Type.DOUBLE, new_gain)]
        )
...
def main(args=None):
    ...
    welding_modular_control_node = WeldingModularControlNode(
        "welding_modular_control_node"
    )
    try:
