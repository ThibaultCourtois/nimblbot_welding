...
from interface_custom_msgs.msg import (
    TelesoudInstruction,
    RobotData,
    Command,
    CommandStatus,
...
class TranslatorNode(Node):
    def __init__(self) -> None:
        ...
        self.instruction_timer = self.create_timer(
            0.02, self.process_pending_instruction
        )
    ...
    def _create_publishers(self) -> None:
        ...
        """
        self.command_pub = self.create_publisher(Command, "/translator/command", 10)

        self.robot_data_pub = self.create_publisher(
            RobotData, "/translator/robotData", 10
        )
            ...
            match instruction:
                ...
                case 7:
                    ...
                    self.get_logger().debug(
                        "Instruction SET DYNAMIC CARTESIAN MOVEMENT"
                    )

                    try:
                        if tcp_speed_vector is not None and len(pose1) > 0:
                            self._command_msg.speed_vector.linear.x = tcp_speed_vector[
                                0
                            ]
                            self._command_msg.speed_vector.linear.y = tcp_speed_vector[
                                1
                            ]
                            ...
                            self.get_logger().error(
                                "Invalid speed vector -must contain 6 elements"
                            )
                            return
                ...
                case 8:
                    ...
                    self.get_logger().debug(
                        "Instruction START DYNAMIC CARTESIAN MOVEMENT"
                    )
                ...
                case 15 | 16:
                    ...
                    try:
                        if pose1 is not None and len(pose1) > 0:
                            self._command_msg.target_pose = (
                                self._construct_command_pose(pose1)
                            )
                        else:
                            self.get_logger().error(
                                "Invalid pose - must contain 6 elements"
                            )
    ...
    def _on_command_status(self, msg: CommandStatus) -> None:
        ...
        if command_id in self.pending_commands:
            if msg.success:
                self.get_logger().debug(
                    f"Command {msg.command_type} executed successfully: {msg.message}"
                )
            else:
                self.get_logger().error(
                    f"Command {msg.command_type} failed: {msg.message}"
                )
            ...
            self.get_logger().warning(
                f"Received status for unknown command ID: {command_id}"
            )
